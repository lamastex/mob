...
\and Hamilton Institute, Maynooth University, Maynooth, Ireland. \\
...
In this paper, we formalize the concept of a {\em sufficient sanitizer}. A sanitizer, i.e.~a mapping of the raw dataset to a sanitized dataset, is called an anonymizer if its sanitization is aimed at preserving anonymity.  
We show that SwapMob, our sanitization method based on swapping segments for trajectory data, is a {\em sufficient sanitizer/anonymizer} for various statistical decision problems, as it not only preserves the aggregate information of the spatial database in the form of sufficient statistics but also provides privacy to the individuals. 
