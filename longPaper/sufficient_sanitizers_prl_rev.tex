...
A {\em sanitizer} $\mathcal{S}$ is a randomized algorithm from the data space $\mathbb{D}$ to a sanitized data space $\widetilde{\mathbb{D}}$, i.e., $\mathcal{S}(d): \mathbb{D} \to \widetilde{\mathbb{D}}$, in such a way that the data in $\widetilde{\mathbb{D}}$ has additional privacy guarantees than in $\mathbb{D}$.
...
\begin{algorithm}[t]
\SetAlgoNoLine
\KwIn{Trajectory database $d=(T_i)$ and partitions $(\tau_j)$ and $(\chi_k)$}
\KwOut{Swapped trajectories $\widetilde{d} = \mathcal{S}(d)$}
\SetKwInput{kwInit}{Initialization}
\kwInit{$\widetilde{d} \gets d$}
...
Update $\widetilde{d}$ by swapping partial trajectories $(T_x)$ between users $x\in S_{\tau_j, \chi_k}$\\
}    
\KwRet{Swapped trajectories $\widetilde{d}$}
