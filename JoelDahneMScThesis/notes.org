...
#+INCLUDE: "code/SwapMob.py" src python
...
benefit is that the relation =being similar= becomes transitive and
...
** Strict or not strict inequalities
...
*** Similarity
When defining what is means for two measurements to be =similar= I
...
we redefine what it means for two measurements to be =similar= and
when a swap is =valid=. Most definitions come naturally but we have to
...
*Remarks:* The precise choice of the partition is highly dependent on
...
is that =being similar= becomes an equivalence relation on \(L \times
...
below.
** Swaps
We could define a swap to be =valid= in exactly the same way as before
...
*Remarks:* One difference in this definition compared to the previous
...
*Remarks:* One other option would be to only allow trajectories to
...
relation =being similar=. Swaps will then be performed on each of
these groups.
*** Order of swaps
...
*Remarks*: I'm fairly certain that the above is correct. I have a
...
T-drive data can be found in [[file:code/T-drive/swapmob.py]]. It begins
by parsing the data and saves it in a more compact form. It then
computes the set of all =valid= swaps and finally it performs all of
these swaps. The interesting parts are computing the set of all
=valid= swaps and performing them, they are explained in a bit more
details below.
*** Computing the set of swaps
The function responsible for computing the set of swaps is ~swaps~. It
...
id_1)\) and \((x_2, y_2, t_2, id_2)\), are =similar= if \(x_1 = x_2\),
